...
def get_catalyst_service_matrix() -> dict:
    ...
    services = [
        ...
        CatalystService(6, "Relational database", "Catalyst Data Store",
                        ...
                        f"The request-scoped SDK probes Catalyst table `{_resource('CATALYST_DATASTORE_TABLE_FIRS', 'namma_ksp_firs')}`; normalized ER analytics retain a local SQLite continuity path.",
                        ["/api/catalyst/datastore/status", "/api/health"], ["CATALYST_DATASTORE_TABLE_FIRS", "CATALYST_DATASTORE_ENABLED"]),
        ...
        CatalystService(10, "Full-text search", "Catalyst Data Store",
                        ...
                        "Global search calls Catalyst Search first across indexed FIR fields and returns a parameterized SQL result only when the managed call is unavailable.",
                        ["/api/search/global"], ["CATALYST_DATASTORE_SEARCH_ENABLED", "CATALYST_SEARCH_TABLE_FIRS"]),
        CatalystService(11, "Text LLMs/RAG/knowledge bases", "Catalyst QuickML",
                        "active" if _resource("NAMMAKSP_QUICKML_ENDPOINT_ID", "") and _resource("NAMMAKSP_QUICKML_ENDPOINT_KEY", "") else ("published" if _resource("NAMMAKSP_QUICKML_ENDPOINT_ID", "") else ("adapter-ready" if _enabled("NAMMAKSP_QUICKML_ENABLED") else "console-initialized")),
                        "QuickML provides a published FIR-status endpoint; the runtime verification performs a real prediction before reporting it active.",
                        ["/api/quickml/predict", "/api/chat"], ["NAMMAKSP_QUICKML_ENDPOINT_ID", "NAMMAKSP_QUICKML_ENDPOINT_KEY"]),
