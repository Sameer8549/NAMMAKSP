...
import { Activity, AlertTriangle, CheckCircle2, Cpu, Database, Edit3, FileClock, RefreshCw, Save, Search, ShieldCheck, Trash2, UserPlus, Users, X } from '../common/icons';
import { ResponsiveContainer, BarChart, Bar, CartesianGrid, XAxis, YAxis, Tooltip } from '../charts/motion';
...
function auditCategory(event: ApiRecord): string {
  const action = String(valueOf(event, ['action', 'event_type'], 'OTHER')).toUpperCase();
  if (/LOGIN|LOGOUT|AUTH|PASSWORD/.test(action)) return 'Authentication';
  if (/ALERT|WARNING/.test(action)) return 'Alert handling';
  if (/FORECAST/.test(action)) return 'Forecast review';
  if (/USER|ROLE|ACCESS/.test(action)) return 'User administration';
  if (/AI|CHAT|LLM|TRANSLAT|TTS/.test(action)) return 'AI services';
  if (/REPORT|EXPORT|PDF|CSV/.test(action)) return 'Reports and exports';
  if (/CASE|FIR|EVIDENCE/.test(action)) return 'Case operations';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onOpenChatDrawer }) => {
  ...
  const [resetRequests, setResetRequests] = useState<ApiRecord[]>([]);
  ...
  const refresh = async () => {
    setLoading(true);
    setError('');
    const results = await Promise.allSettled([apiClient.systemStatus(), apiClient.catalystServices(), apiClient.listUsers(), apiClient.listAudit(), apiClient.listPasswordResetRequests()]);
    ...
    if (results[4].status === 'fulfilled') setResetRequests(results[4].value);
  ...
  const serviceRows = useMemo(() => {
    ...
    const operational = /^(active|configured|published|local|demo-fallback)$/i;
    const requiredCapabilities = /backend logic|managed runtime|frontend|database|storage|cache|search|LLMs|authentication|scheduled jobs|in-project events|voice\/translation/i;
    return (rows as ApiRecord[]).filter(item => {
      const status = String(valueOf(item, ['status', 'state'], ''));
      const capability = String(valueOf(item, ['capability', 'feature', 'name'], ''));
      return operational.test(status) && requiredCapabilities.test(capability);
    });
  }, [services]);
  const activeServices = serviceRows.filter(item => String(valueOf(item, ['status', 'state'], '')).match(/active|live|ready|configured|enabled|local|demo-fallback/i)).length;
  ...
  const roleChart = Object.entries(users.reduce<Record<string, number>>((counts, user) => { const role = String(valueOf(user, ['role', 'user_role'], 'Unassigned')); counts[role] = (counts[role] || 0) + 1; return counts; }, {})).map(([role, count]) => ({ role, count, percentage: users.length ? Number((count / users.length * 100).toFixed(1)) : 0 }));
  const auditActionChart = Object.entries(audit.reduce<Record<string, number>>((counts, event) => { const category = auditCategory(event); counts[category] = (counts[category] || 0) + 1; return counts; }, {})).map(([category, count]) => ({ category, count, percentage: audit.length ? Number((count / audit.length * 100).toFixed(1)) : 0 })).sort((a, b) => b.count - a.count);
  const aiEvents = audit.filter(event => /AI|CHAT|LLM|TRANSLAT|TTS/i.test(String(valueOf(event, ['action', 'event_type'], ''))));
  const openUserReview = () => onOpenChatDrawer('Review the current identity and role registry. Use the loaded users, password-assistance queue, audit trail, and Catalyst service status. Tell me the highest administrative risks and the next actions.');
  ...
  const overview = <>
    ...
    <div className="admin-overview-grid">
      <section style={card}>
        <h2 className="admin-section-title">Account share by role</h2><p className="admin-section-copy">Percentage of all {users.length} accounts in the current user-registry response</p>
        <div style={{ height: 250 }}><ResponsiveContainer width="100%" height="100%"><BarChart data={roleChart} margin={{ left: 0, right: 8, top: 8, bottom: 0 }}><CartesianGrid stroke="var(--border)" vertical={false} /><XAxis dataKey="role" tick={{ fill: 'var(--text-secondary)', fontSize: 11 }} /><YAxis tick={{ fill: 'var(--text-secondary)', fontSize: 11 }} /><Tooltip contentStyle={{ background: 'var(--surface-elevated)', border: '1px solid var(--border)', color: 'var(--text-primary)' }} /><Bar {...chartMotion} dataKey="percentage" name="Share of accounts (%)" fill="var(--accent)" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></div>
        <div className="admin-chart-breakdown">{roleChart.map(item=><span key={item.role}><strong>{item.role}</strong><b>{item.percentage}%</b><small>{item.count} of {users.length}</small></span>)}</div>
      </section>
      <section style={card}>
  ...
  const userTable = <section style={card}>
    <div className="admin-section-heading"><div><h2 className="admin-section-title">Identity and role registry</h2><p className="admin-section-copy">Create accounts, change roles, and suspend access through the authenticated administration API.</p></div><div className="admin-inline-actions"><button className="btn btn-secondary" onClick={openUserReview}><ShieldCheck size={15} /> Review with assistant</button><button className="btn btn-primary" onClick={() => setUserEditor({ username: '', password: '', role: 'Investigator', active: true, isNew: true })}><UserPlus size={15} /> Add user</button></div></div>
    <div className="admin-table-wrap"><table className="admin-data-table"><thead><tr>{['Username', 'Role', 'Status', 'Last activity', 'Actions'].map(label => <th key={label}>{label}</th>)}</tr></thead><tbody>{filteredUsers.map((user, index) => { const username=String(valueOf(user, ['username', 'user_id', 'id'], index)); const role=String(valueOf(user, ['role', 'user_role'], 'Investigator')); const active=Boolean(valueOf(user, ['active', 'is_active'], true)); return <tr key={username}><td data-label="Username"><strong>{username}</strong></td><td data-label="Role">{role}</td><td data-label="Status"><span className={`admin-state-chip ${active ? 'is-active' : 'is-disabled'}`}>{active ? 'Active' : 'Disabled'}</span></td><td data-label="Last activity">{String(valueOf(user, ['last_login', 'updated_at'], 'No session activity'))}</td><td data-label="Actions"><div className="admin-inline-actions"><button className="btn btn-secondary" onClick={() => setUserEditor({ username, password: '', role, active, isNew: false })}><Edit3 size={14}/> Edit</button><button className="btn btn-secondary" onClick={() => void apiClient.updateUser(username,{active:!active}).then(refresh).catch(reason=>setError(reason.message))}>{active ? 'Disable' : 'Enable'}</button><button className="btn btn-danger" onClick={() => void removeUser(username)}><Trash2 size={14}/>{pendingDelete===username?'Confirm':'Delete'}</button></div></td></tr>; })}</tbody></table></div>
    <div className="admin-reset-queue"><h3>Password assistance requests <span className="admin-state-chip is-active">{resetRequests.filter(item => String(valueOf(item,['status'],'open')) === 'open').length} open</span></h3>{resetRequests.length ? resetRequests.slice(0,20).map((item,index) => { const id=Number(valueOf(item,['id'],index)); const status=String(valueOf(item,['status'],'open')); return <div className="admin-reset-row" key={id}><span><strong>{String(valueOf(item,['identifier'],'Unknown identifier'))}</strong><small>{String(valueOf(item,['requested_at'],'Time unavailable'))}</small></span><span className={`admin-state-chip ${status === 'open' ? 'is-active' : ''}`}>{status}</span>{status === 'open' && <button className="btn btn-secondary" onClick={() => void apiClient.resolvePasswordResetRequest(id).then(refresh).catch(reason=>setError(reason.message))}>Mark reviewed</button>}</div>; }) : <div className="admin-empty">No password assistance requests.</div>}</div>
  </section>;
  ...
  const auditList = <section style={card}>
    ...
    <div className="admin-overview-grid"><div className="admin-audit-list">{filteredAudit.slice(0, 100).map((event, index) => <button className="admin-audit-row" onClick={() => setAuditDetail(event)} key={String(valueOf(event, ['id', 'audit_id'], index))}><span>{String(valueOf(event, ['timestamp', 'created_at'], 'Timestamp unavailable'))}</span><strong>{String(valueOf(event, ['actor', 'username'], 'System'))}</strong><span>{String(valueOf(event, ['action', 'event_type'], 'AUDIT_EVENT'))}</span><span>{String(valueOf(event, ['target_resource', 'resource', 'detail'], 'Governance record'))}</span></button>)}</div><div style={card}><h3 className="admin-section-title">Audit share by operational category</h3><p className="admin-section-copy">Percentage of all {audit.length} events in the current audit response</p><div style={{height:285}}><ResponsiveContainer width="100%" height="100%"><BarChart data={auditActionChart} layout="vertical" margin={{left:8,right:18,top:8,bottom:8}}><CartesianGrid stroke="var(--border)" horizontal={false}/><XAxis type="number"/><YAxis type="category" dataKey="category" width={122}/><Tooltip contentStyle={{background:'var(--surface-elevated)',border:'1px solid var(--border)'}}/><Bar {...chartMotion} dataKey="percentage" name="Share of audit events (%)" fill="var(--info)" radius={[0,4,4,0]}/></BarChart></ResponsiveContainer></div><div className="admin-chart-breakdown">{auditActionChart.map(item=><span key={item.category}><strong>{item.category}</strong><b>{item.percentage}%</b><small>{item.count} of {audit.length}</small></span>)}</div></div></div>
  </section>;

  const serviceGrid = <div className="admin-service-grid">{serviceRows.map((service, index) => { const status = String(valueOf(service, ['status', 'state'], 'reported')); const name=String(valueOf(service, ['name', 'service'], `Catalyst service ${index + 1}`)); return <section key={name} style={card}><div className="admin-service-title"><strong>{name}</strong>{status.match(/active|live|ready|enabled|configured|local|demo-fallback/i) ? <CheckCircle2 size={17} color="var(--success)" /> : <AlertTriangle size={17} color="var(--warning)" />}</div><div className="admin-service-state">{status}</div><div className="admin-section-copy">{String(valueOf(service, ['detail', 'description', 'mode'], 'State reported by the backend service registry.'))}</div><button className="btn btn-secondary" style={{marginTop:'.8rem',width:'100%'}} onClick={()=>setAuditDetail({service:name,...service})}>Inspect service</button></section>; })}{!serviceRows.length && <section style={card}>Catalyst service detail is unavailable. The backend returned no service records.</section>}</div>;

  const aiUsage = (() => { const usageByAction=Object.entries(aiEvents.reduce<Record<string,number>>((acc,event)=>{const key=String(valueOf(event,['action','event_type'],'AI_REQUEST'));acc[key]=(acc[key]||0)+1;return acc;},{})).map(([action,count])=>({action,count})); return <><div className="admin-health-grid"><Metric icon={<Cpu size={19} />} label="AI audit events" value={aiEvents.length} note="Observed AI, chat, translation and voice actions" /><Metric icon={<Activity size={19} />} label="Last AI activity" value={String(valueOf(aiEvents[0] || {}, ['timestamp','created_at'], 'No audited call'))} note="Most recent role-authenticated model action" /><Metric icon={<ShieldCheck size={19} />} label="Guardrail mode" value="Verified data only" note="Entity checks precede model context" /></div><section style={{...card,minHeight:320}}>{usageByAction.length ? <ResponsiveContainer width="100%" height={300}><BarChart data={usageByAction}><CartesianGrid stroke="var(--border)" vertical={false}/><XAxis dataKey="action"/><YAxis allowDecimals={false}/><Tooltip/><Bar {...chartMotion} dataKey="count" fill="var(--accent)" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer> : <div className="admin-empty">No AI activity is present in the current audit window. Use the role assistants, then refresh this view to inspect usage.</div>}</section></>; })();

  return <div className="admin-command-center">
    <header className="admin-header-banner"><div className="admin-header-title"><div className="admin-header-badge"><ShieldCheck size={22} /></div><div className="admin-header-text"><h1>Platform Governance</h1><p>{loading ? 'Loading authenticated governance sources…' : `${users.length} accounts · ${audit.length} audited actions · ${activeServices}/${serviceRows.length || 0} services operational`}</p></div></div><div className="admin-header-status-group"><span className={`admin-status-pill ${error ? '' : 'admin-status-pill--success'}`}><span className="admin-live-dot" /> {loading ? 'Checking backend' : error ? 'Partial data' : 'Backend connected'}</span><button style={actionButton} onClick={() => void refresh()} disabled={loading}><RefreshCw size={14} /> {loading ? 'Refreshing' : 'Refresh'}</button></div></header>
    ...
    {view === 'Overview' && <><RoleAnalyticsWorkspace role="ADMIN" />{overview}</>}{view === 'User Management' && userTable}{view === 'Security Audit' && auditList}{view === 'System Health' && serviceGrid}{view === 'AI Usage' && aiUsage}
    ...
    <style>{`.admin-inline-actions{display:flex;align-items:center;gap:.4rem;flex-wrap:wrap}.admin-state-chip{display:inline-flex;padding:.25rem .45rem;border:1px solid var(--border);border-radius:var(--radius-sm);font-size:.68rem;font-weight:800}.admin-state-chip.is-active{color:var(--success);background:var(--success-bg)}.admin-state-chip.is-disabled{color:var(--critical);background:var(--critical-bg)}.admin-audit-row{width:100%;text-align:left;color:var(--text-secondary);background:var(--surface-muted)}.admin-chart-breakdown{display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:.45rem}.admin-chart-breakdown>span{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:.1rem .5rem;padding:.5rem .6rem;border:1px solid var(--border);border-radius:var(--radius-sm);background:var(--surface-muted);font-variant-numeric:tabular-nums}.admin-chart-breakdown strong{overflow:hidden;text-overflow:ellipsis;font-size:.7rem;color:var(--text-primary)}.admin-chart-breakdown b{font-size:.75rem;color:var(--info)}.admin-chart-breakdown small{grid-column:1/-1;color:var(--text-muted);font-size:.64rem}.admin-dialog-backdrop{position:fixed;inset:0;z-index:700;background:rgba(4,14,29,.72);display:grid;place-items:center;padding:1rem}.admin-dialog{width:min(560px,100%);max-height:90vh;overflow:auto;display:grid;gap:1rem;padding:1rem;background:var(--surface-elevated);border:1px solid var(--border);border-radius:var(--radius-lg);box-shadow:var(--shadow-lg)}.admin-dialog header,.admin-dialog footer{display:flex;align-items:center;justify-content:space-between;gap:1rem}.admin-dialog h2{margin:0;font-size:1.05rem}.admin-dialog p{margin:.2rem 0 0;color:var(--text-muted)}.admin-dialog label{display:grid;gap:.35rem;color:var(--text-secondary);font-size:.75rem;font-weight:750}.admin-dialog input,.admin-dialog select{width:100%;padding:.65rem;border:1px solid var(--border);border-radius:var(--radius-md);background:var(--surface-muted);color:var(--text-primary)}.admin-check{display:flex!important;grid-template-columns:auto 1fr;align-items:center}.admin-check input{width:auto}.admin-detail-list{display:grid;gap:.55rem}.admin-detail-list>div{padding:.7rem;border:1px solid var(--border);background:var(--surface-muted);border-radius:var(--radius-md)}.admin-detail-list dt{font-size:.66rem;text-transform:uppercase;color:var(--text-muted)}.admin-detail-list dd{margin:.2rem 0 0;overflow-wrap:anywhere;color:var(--text-primary)}`}</style>
  </div>;
};
