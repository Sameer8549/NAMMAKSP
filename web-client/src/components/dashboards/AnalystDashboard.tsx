...
import { Activity, FileText, MapPin, Network, Search, ShieldCheck } from '../common/icons';
import { Area, AreaChart, Bar, BarChart, CartesianGrid, Legend, ResponsiveContainer, Tooltip, XAxis, YAxis } from '../charts/motion';
...
import { AnalystAnalyticsWorkbench } from '../analytics/AnalystAnalyticsWorkbench';
import '../analytics/analystLegacyOverride.css';

interface AnalystDashboardProps { onOpenExplainModal: () => void; onOpenChatDrawer: (prompt?: string) => void; }
...
export const AnalystDashboard: React.FC<AnalystDashboardProps> = ({ onOpenChatDrawer }) => {
  ...
  useEffect(() => {
    ...
    else if (view.includes('modus')) setTab('MO'); else if (view.includes('seasonal')) setTab('SEASONAL'); else if (view.includes('risk') || view.includes('financial')) setTab('RISK_FINANCIAL');
    else setTab('TRENDS');
  }, [activeView]);
  ...
  return <div className="analyst-workspace">
    ...
    {tab !== 'TRENDS' && <section className="analyst-scope" aria-label="Analytical scope">
    ...
    {tab === 'TRENDS' && <AnalystAnalyticsWorkbench onOpenCase={setCaseDetail} />}
